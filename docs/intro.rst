...
In addition to the command-line tool, there is also an :ref:`API <api>` and web application. However, in that case publication(s) have to be manually supplied (instead of automatic selection from Europe PMC) and publications are given and a bio.tools entry is returned for one tool at a time. In addition to a publication, web pages and the tool name can be supplied to help the Pub2Tools algorithm. The :ref:`Pub2Tools API <api>` can be consumed through the :ref:`/api <api_endpoint>` endpoint, either by sending requests to the public instance https://elixir.ut.ee/pub2tools/api or by sending requests to a local instance set up by following the instructions under :ref:`server`. The API could be used for example for automatic filling of fields in a new tool registration interface for bio.tools.
...
For slightly better results, before a bigger run of Pub2Tools, it could be beneficial to `test if PubFetcher scraping rules <https://pubfetcher.readthedocs.io/en/stable/scraping.html#testing-of-rules>`_ are still up to date. Also, publisher web sites have to be consulted sometimes, so it could be beneficial to run Pub2Tools in a network with good access to journal articles.
...
.. code-block:: bash
  ...
  $ java -jar path/to/pub2tools-cli-<version>.jar -all output \
...
For testing out one bio.tools entry generation from a given publication, the public web application at https://elixir.ut.ee/pub2tools/ can be used by filling in the "Publications" and clicking on "Run all". As for using the API, there are a few :ref:`api_examples`.
